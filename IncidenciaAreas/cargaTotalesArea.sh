...
wget -O ${CARPETA}/hot-config.txt ${CARPETA}/sergasConfig.html https://coronavirus.sergas.es/datos/libs/hot-config/hot-config.txt
...
SUFIJO=$(cat hot-config.txt  |grep CifrasTotais | grep URL | awk '{l=split($0,datos,"}");print datos[2];}' | sed 's/",//g')
...
SUFIJO=$(cat hot-config.txt  |grep InfectadosPorFecha | grep URL | awk '{l=split($0,datos,"}");print datos[2];}' | sed 's/",//g')
echo "Se cargan datos de: https://coronavirus.sergas.gal/infodatos/${AYER_DEC}${SUFIJO}"
...
# Al tratarse de un fichero acumulado, el último descargado introduce cambios en cualquier fecha anterior
SUFIJO=$(cat hot-config.txt  |grep PorcentajeInfecciones | grep URL | awk '{l=split($0,datos,"}");print datos[2];}' | sed 's/",//g')
echo "Se cargan datos de: https://coronavirus.sergas.gal/infodatos/${AYER_DEC}${SUFIJO}"
...
# Es un fichero incremental, se concatena al historico
SUFIJO=$(cat hot-config.txt  |grep OcupacionCamas | grep URL | awk '{l=split($0,datos,"}");print datos[2];}' | sed 's/",//g')
echo "Se cargan datos de: https://coronavirus.sergas.gal/infodatos/${AYER_DEC}${SUFIJO}"
